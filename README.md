...
# 実行
demos/box2d/lua_b2 demos/box2d/main.lua
```

### ビルド手順 (Windows)

Windows では CMake でビルドする。
...
# 2. Box2D をビルド
cmake -S deps/box2d -B deps/box2d/build -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF
cmake --build deps/box2d/build --config Release

# 3. b2lua バインディング + カスタム Lua バイナリをビルド
cl /O2 /c /I deps\lua /I deps\box2d\include demos\box2d\b2lua.c /Fo:build\b2lua.obj
```

カスタム main.c を作成して Box2D モジュールをプリロードする Lua バイナリをリンクする。
詳細は `demos/box2d/build.sh` の手順を参考に MSVC 向けに読み替える。
...
# 5. 実行
demos\box2d\lua_b2.exe demos\box2d\main.lua
```
