...
tcs 内部を「bound tree → IL → Lua 出力」に再編し、**IL を意味論の正本**にする。
...
現行構成は C# →（bound tree 走査）→ Lua で、中間に明示的な表現がない。
...
| 段階 | 内容 | 完了条件 |
|---|---|---|
| M1 | Transpiler 内部を bound tree→IL→Lua に再編 | 挙動不変。既存テスト全通過。source module 単位 emit（増分コンパイル設計）と両立 |
