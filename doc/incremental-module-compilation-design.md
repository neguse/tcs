...
この変更で「chunk 再実行により dirty フラグ static が初期値 true へ戻る」ことを reload 検知に使う既存イディオム (lub の SDF 系サンプル) は成立しなくなる。代替として、hot apply の commit 後に registry が entry type の `onReload()` (存在すれば) を呼ぶ。コード由来の derived data (SDF mesh 等) の再構築はこの hook で明示する。fresh VM / restart では呼ばない (onInit が担う)。onReload の error は commit を巻き戻さず警告に留める。
...
tcs runtime 側の hot apply はこの `ModuleRegistry` transaction だけが担う (かつて runtime にあった legacy `HotReload.swap` は consumer 不在のため削除済み。backlog T154 も同時にクローズ)。lub の entry reload は引き続き `lume.hotswap` が担い、tcs の file watcher と二重に動かさない。
...
- module browser path は `applyBatch` を使う (legacy `HotReload.swap` は削除済み)。
- authoritative CLI/check/CI は full diagnostics のままにする。

module ABI 公開前に T151 の中央命名、T155 の entry 解決を統合する。T149 の source-order-independent inheritance は declare/define 分離で満たす (T154 は legacy HotReload runtime の削除によりクローズ)。**T149/T151/T155 は未着手**であり、本設計の前提としてこの積み残しを実装計画に織り込む。
...
- T149 の acceptance と registry-owned transaction cases を取り込む。
