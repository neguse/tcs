...
- interface は型検査の痕跡のみで実行時表現を持たない。interface への
  type test は v0 では未決（付録 C）
...
- type test（`is T`）: 実行時型が T またはその派生なら真。null は常に偽
  （現行バグ T222 はこの規範で修正）
...
- string: **不変のバイト列（UTF-8）**。Length・index・IndexOf 等は
  バイト単位。C# frontend（UTF-16 code unit）との差は known-differences が
  正本（support-matrix §27）。
  判断: 出荷ターゲットの表現（バイト列）を規範に置き、開発時 .NET 実行を
...
- 種別: 除数 0 (§5)、i32 除算 overflow (§5)、f32→i32 変換不能 (§5)、
  ...
  明示 cast の失敗 (§9)、runtime intrinsic が定義する error
...
- `-ffp-contract=off`、fast-math 系最適化の禁止 (§6)
...
- interface への type test の扱い（診断化 or 実行時表現の付与）
- Nullable<T> の位置づけ
