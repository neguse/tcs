...
- 進捗 (2026-06-29): `run-tests.sh` / `run-tests.ps1` で analyzer nupkg を pack し、一時 project から `PackageReference` で参照して TCS1001 x4 / TCS1002 x1 / TCS1003 x1 が出ることと `.editorconfig` による TCS1002 error override が build に反映されることを検証
- 進捗 (2026-06-29): README / analyzer-demo README に `PackageReference` 導入手順と local nupkg consumer 検証の位置づけを追加
- 進捗 (2026-06-29): JetBrains InspectCode 2026.1.3 headless 実行で ProjectReference analyzer-demo と local nupkg `PackageReference` consumer の TCS1001 x4 / TCS1002 x1 / TCS1003 x1、および PackageReference consumer の TCS1002 error override を確認し、`samples/analyzer-demo/verify-inspectcode.sh` に手順を script 化
