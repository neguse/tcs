...
- T122: Rider リアルタイム警告向け tcs Roslyn Analyzer PoC を作る
  ...
  - `.ps1` は pwsh 7 の read-only 自動変数 (`$IsWindows` / `$IsMacOS`) と衝突する変数名を使わない。`run-tests.ps1` の bash gate は WSL の `System32\bash.exe` を除外し、script path は forward slash で渡す
