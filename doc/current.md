...
### 完了済み (554テスト tcs / 47テスト analyzer / 477テスト lub3d)
...
**T138**: supported API allowlist を完全シグネチャ単位へ変更 (未実装 overload の TCS1002 検出、Analyzer/check parity matrix)
...
| custom property | get_/set_ メソッド (全使用サイトで accessor 呼び出し) |
...
| &= \|= ^= <<= >>= | 展開 x = x op y |
| int / int, int % int | __tcs_idiv / __tcs_irem (0 方向 truncation) |
| float % float | math.fmod |
| s += x (string) | s = s .. x |
...
- P0: 2026-07-12全体コードレビューで確認したsilent wrong-codeの修正 (T149-T153) + T180 (値型 is パターンの nil マッチ、2026-07-17 発見)。棚卸し (2026-07-17) で T154 クローズ / T163 削除 / T153 縮小。T138-T148 / T151 は完了
- 着手順: T149-T150 (継承)、T152-T153 (runtime 契約)、T180 は並行可。継承 T149-T150、T180 は並行可
